  ...
  constructor(
    ...
    if (pageSize <= 0) {
      throw new ValidationError(
        `Page size ${pageSize} must be a positive number higher than 0`,
      )
    }
    ...
    if (currentPage <= 0) {
      throw new ValidationError(
        `Current page ${currentPage} must be a positive number higher than 0`,
      )
    }
    ...
    this.nextPage =
      this.pages === this.currentPage ? this.currentPage : this.currentPage + 1
  }
}
